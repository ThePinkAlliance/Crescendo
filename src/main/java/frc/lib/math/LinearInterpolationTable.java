...
public class LinearInterpolationTable {
    ...
    public LinearInterpolationTable(List<Pair<Double, Double>> points) {
        ArrayList<Pair<Double, Double>> mutList = new ArrayList<>(points);
     ...
     */
    public List<Pair<Double, Double>> streamListVector(Stream<Pair<Double, Double>> stream) {
        Iterator<Pair<Double, Double>> iterator = stream.iterator();
    ...
    public double interp(double e) {
         ...
         */
        ArrayList<Pair<Double, Double>> greaterPoints = new ArrayList<>(
                streamListVector(
                        points.stream()
                                .filter(v -> Math.abs(v.getFirst()) > Math.abs(e)
                                        && Math.signum(e) == Math.signum(v.getFirst()))));
        ArrayList<Pair<Double, Double>> smallerPoints = new ArrayList<>(
                streamListVector(
                        points.stream()
                                .filter(v -> Math.abs(v.getFirst()) < Math.abs(e)
                                        && Math.signum(e) == Math.signum(v.getFirst()))));
        ...
        Pair<Double, Double> vec1 = smallerPoints.get(0);
        Pair<Double, Double> vec2 = greaterPoints.get(0);
